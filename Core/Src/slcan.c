...
static uint8_t slcan_can_set_baud_rate( uint8_t* cmd_buff)
{
    ...
    {
        for( i = 0u; i < (sizeof(slcan_baud_rate_map)/sizeof(slcan_baud_rate_map[0])); i++ )
        {
            if(*cmd_buff == slcan_baud_rate_map[i].cmd)
            {
                hcan1.Init.Prescaler = slcan_baud_rate_map[i].prescaler;
                ret = HAL_OK;
                break;
            }
        }
...
static uint8_t slcan_can_get_version( uint8_t* cmd_buff)
{
    CDC_Transmit_FS((uint8_t *)slcan_version,sizeof(slcan_version));
    uint8_t ret;
    if( *cmd_buff != '\r' )
    {
        ret = HAL_ERROR;
...
void SLCAN_pc_cmd_process(char cmd, uint8_t* cmd_buff)
{
    ...
    for(i = 0u; i < sizeof(slcan_cmd_table_c)/sizeof(slcan_cmd_table_c[0]);i++)
    {
        ...
        {
            (void)slcan_cmd_table_c[i].cmd_fun( cmd_buff );
            break;
        }
    }
